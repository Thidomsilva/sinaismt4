...
//| + Integração com API Signals Live
//+------------------------------------------------------------------+
#property strict
...
// Avaliação de Assertividade (Backtest Rápido)
...
extern bool     EntryOnBarClose     = true; // Para avaliação de binárias
...
// Aparência das Linhas de Sincronização
...
datetime lastSentBarTime = 0; // Para garantir envio único por vela

// URL da API - Substitua pela sua URL real da Vercel/Firebase
string apiUrl = "https://sinaismt4.vercel.app/api/snapshot";
...
// --- Funções de Avaliação de Assertividade ---
...
void SendRequest(string jsonData) {
    ...
    int res = WebRequest("POST", apiUrl, headers, 5000, data, result, headers);

    if (res == -1) {
        Print("Erro no WebRequest: ", GetLastError());
    } else {
        // Print("Sinal enviado. Resposta do servidor: ", CharArrayToString(result));
    }
}

//+------------------------------------------------------------------+
//| Funções Principais do Indicador                                  |
//+------------------------------------------------------------------+
int init() {
...
int start() {
    // --- Lógica de Execução Única por Vela ---
    if (Time[0] == lastSentBarTime) {
        return (0); // Já processou esta vela, sai
    }
    lastSentBarTime = Time[0];

    // --- Limpeza e Desenho do Painel ---
    ArrayInitialize(BuyBuf, EMPTY_VALUE); ArrayInitialize(SellBuf, EMPTY_VALUE);
    int idx = 0, x0 = X, y0 = Y;
    ...
    for (int j = 0; j < nTF; j++) {
        string cap = PeriodToString(TFs[j]);
    ...
    for (int r = 0; r < nf; r++) {
        ...
        for (int j = 0; j < nTF; j++) {
            int v = VotesForTF(Symbol(), TFs[j], 1); // usa a vela fechada
    ...
    // --- Geração e Desenho do Sinal na Vela Fechada ---
    ...
    // --- Alertas ---
    if (EnableAlerts && IsMarketOpen() && sig != 0 && Time[1] != lastAlertBar) {
    ...
    double wr = (total > 0 && (wins+loss)>0) ? 100.0 * wins / (wins+loss) : 0.0;
    ...
    // --- COLETA DE DADOS E ENVIO PARA API ---
    string finalSignal;
    if(sig == 1) finalSignal = "BUY";
    else if(sig == -1) finalSignal = "SELL";
    else finalSignal = "NONE";
    
    string notes = "";
    int nf_votes = 0;
    if(UseMAVotes) { notes = notes + "MA "; nf_votes++;}
    if(UseCCI)     { notes = notes + (nf_votes > 0 ? "; " : "") + "CCI"; nf_votes++; }
    if(UseMACD)    { notes = notes + (nf_votes > 0 ? "; " : "") + "MACD"; nf_votes++; }
    if(UseADX)     { notes = notes + (nf_votes > 0 ? "; " : "") + "ADX"; nf_votes++; }
    if(UseStoch)   { notes = notes + (nf_votes > 0 ? "; " : "") + "Stoch"; }
    ...
    string jsonData = StringFormat(
        ...
        ExpiryCandles,
        Time[1],
        (IsMarketOpen() ? "true" : "false"), // CORRIGIDO: envia como booleano JSON
        (int)MarketInfo(_Symbol, MODE_SPREAD),
        notes,
        (OnlyOnBarClose ? "true" : "false") // CORRIGIDO: envia como booleano JSON
    );
